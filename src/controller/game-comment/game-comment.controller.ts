...
class GameCommentController {
    ...
    async editReplyComment(req: IRequest, res: Response, next: NextFunction): Promise<void> {
        try {
            const {id} = req.params;
            const {_id} = req.user as IUser;
            const {comment} = req.body as Partial<IGameComment>;

            const commentExist = await gameCommentService.getReplyComment(id);

            if (!commentExist?.replyComments.length || String(commentExist?.replyComments[0].userId) !== _id.toString()) {
            ...
            await gameCommentService.editReplyComment(id, comment as string);

            await historyService.addEvent(
                {
                    event: `${HistoryEvent.editGameComment} with id ${id} (reply for comment  with id ${commentExist?._id} )`,
                    userId: _id
                }
            );
    ...
    async deleteReplyComment(req: IRequest, res: Response, next: NextFunction): Promise<void> {
        try {
            const {id} = req.params;
            const user = req.user as IUser;

            const isCommentExists = await gameCommentService.getReplyComment(id);

            if (!isCommentExists?.replyComments.length) {
                throw new ErrorHandler(
                    ResponseStatusCodeEnum.BAD_REQUEST,
                    customErrors.BAD_REQUEST_COMMENT_IS_NOT_PRESENT.message,
                    customErrors.BAD_REQUEST_COMMENT_IS_NOT_PRESENT.code
            ...
            if (user.role !== UserRoleEnum.ADMIN && String(isCommentExists.replyComments[0].userId) !== String(user._id)) {
            ...
            await gameCommentService.deleteReplyComment(id);
